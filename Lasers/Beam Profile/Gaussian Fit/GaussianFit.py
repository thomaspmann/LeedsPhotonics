...
# File downloaded from: https://github.com/indiajoe/HandyTools4Astronomers/blob/master/GaussianFit.py
...
def moments1D(inpData):
    ...
    Data = np.ma.masked_less(inpData - (bkgC + bkgSlope * np.arange(len(inpData))),
                             0)  # Removing the background for calculating moments of pure gaussian
...
def moments2D(inpData):
    ...
    bkg = np.median(np.hstack((inpData[0, :], inpData[-1, :], inpData[:, 0],
                               inpData[:, -1])))  # Taking median of the 4 edges points as background
...
def FitGauss2D(Data, ip=None):
    ...
    def errfun(ip):
        ...
        Weights = np.sqrt(
            np.square(dXcoords) + np.square(dYcoords))  # Taking radius as the weights for least square fitting
        return np.ravel((Gaussian2D(*ip)(*np.indices(Data.shape)) - Data) / np.sqrt(
            Weights))  # Taking a sqrt(weight) here so that while scipy takes square of this array it will become 1/r weight.
